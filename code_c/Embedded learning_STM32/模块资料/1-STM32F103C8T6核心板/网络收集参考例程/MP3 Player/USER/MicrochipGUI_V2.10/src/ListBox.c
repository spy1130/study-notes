    ...
    L_LB_DRAWITEM:
            ...
            if(GetY() < pLb->hdr.bottom - GOL_EMBOSS_SIZE - LB_INDENT)
                ...
                case LB_STATE_BITMAP:
                    if(pCurItem->pBitmap != NULL)
                    {
				        if((GetY()+((pLb->textHeight-GetImageHeight(pCurItem->pBitmap))>>1))>pLb->hdr.top)	  /* ÐÞ¸´BUG*/
				        if((GetY()+((pLb->textHeight-GetImageHeight(pCurItem->pBitmap))>>1))<pLb->hdr.bottom-16)
                        if
                        (
                            !PutImage
                                (
                ...
                case LB_STATE_TEXT:
		          if( _cursorY >= pLb->hdr.top)	 /* ÐÞ¸´BUG*/
		           if( _cursorY <= pLb->hdr.bottom-16)
    ...
    L_LB_NEXTITEM:
        ...
        case LB_STATE_ITEMFOCUS:
            if(pLb->pFocusItem == pCurItem)
            {
                ...
                temp = GetY();
				if(	 _cursorY>=	pLb->hdr.top&& _cursorY<pLb->hdr.bottom-GOL_EMBOSS_SIZE-LB_INDENT) /* ÐÞ¸´BUG */
                if
                (
                    !Rectangle
                        (
